...
long_data <- read.csv("polyandry_meta_full_dataset.csv") %>% 
...
fecund_data <- read.csv("polyandry_meta_full_dataset.csv") %>%
...
write.csv(fecund_data, "fecund_data.csv")


### GLOBAL EFFECT SIZE PLOT
global_results <- read.csv("global_model_results.csv")
